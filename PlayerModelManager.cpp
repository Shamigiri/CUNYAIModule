...
void Player_Model::readPlayerLog(Player_Model & enemy_player_model) // Function that reads in previous game's data
{
	...
	int numoflines = 0;
	getline(infile, data);// This loop calculates the number of lines the file has
						  //++numoflines;
	...
	while (infile >> data) // Read in the data
	{
		...
		if (data == ",")
			infile >> data;
		//std::cout << "Iteration number " << iteration << endl;
		...
		//std::cout << "oldminstock sub " << index << " is " << oldMinStock[index] << endl;
		...
		//std::cout << "earliesttime sub " << index << " is " << earliestTimeAverage[index] << endl;
		...
		{
			//fill_n(oldMinStock, 29, 0);
			//fill_n(oldEarliestTime, 29, 0);
			//fill_n(oldMaxStock, 29, 0);
			//fill_n(oldMaxTime, 29, 0);
			index = 0;
			std::cout << "index has been reset to 0" << endl;
		}

		iteration++;
	}
	//cout << earliestTimeAverage[1] << " " << maxStockAverage[1] << " " << maxTimeAverage[1] << " ";
...
void Player_Model::writePlayerLog(Player_Model & enemy_player_model, bool gameComplete) { //Function that records a player's noticed inventory
	...
	if (gameComplete)
	{
		...
		if (!inFile)
		{
			...
			for (int i = 0; i < 29; i++)
				earliestDate << left << setw(30) << enemy_player_model.units_.unitInventoryLabel[i];
			earliestDate << endl;
		}
		ofstream earliestDate;
		earliestDate.open(".\\bwapi-data\\write\\" + Broodwar->enemy()->getName() + ".txt", ios_base::app);
